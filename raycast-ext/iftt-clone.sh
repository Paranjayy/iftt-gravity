...
if [ "$1" == "archive" ]; then
  ...
  nohup $BUN src/lib/archive.ts > /tmp/gravity-archive.log 2>&1 &
  disown
...
cd /Users/paranjay/Developer/iftt/raycast-ext && $BUN run build > /dev/null 2>&1
...
nohup $BUN src/lib/bot.ts > /tmp/gravity-bot.log 2>&1 &
disown
...
if lsof -i:3031 >/dev/null && lsof -i:3030 >/dev/null; then
  ...
  echo "🤖 Bot Error: $(tail -n 1 /tmp/gravity-bot.log)"
  exit 1
fi
